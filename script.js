...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function positionNebulaButtons() {
        ...
        const centerX = isMobile ? window.innerWidth / 2 : rect.left + rect.width / 2;
        ...
        const btnSize = isMobile ? 60 : 100;
    projects.forEach((project, index) => {
            const angle = (index / projects.length) * Math.PI * 2;
            
            // Calculate position on the oval
            const x = centerX + radiusX * Math.cos(angle) - btnSize/2;
            const y = centerY + radiusY * Math.sin(angle) - btnSize/2;
            
            // Create button
            const button = document.createElement('button');
            const variant = project.url ? 'nebula-a' : 'nebula-b';
        button.className = `nebula-button ${variant}`;
            ...
            if (words.length === 2) {
              span.innerHTML = `${words[0]}<br>${words[1]}`;
            } else {
              span.textContent = project.title;
            }
            button.appendChild(span);
        // Dynamically shrink font to fit up to 2 lines in the button
        (function adjustFont() {
            const maxWidth = button.clientWidth * 0.8;
            const maxHeight = button.clientHeight * 0.8;
            let fontSize = parseFloat(window.getComputedStyle(span).fontSize);
            span.style.whiteSpace = 'normal';
            span.style.lineHeight = '1.1';
            span.style.wordBreak = 'keep-all';
            while ((span.scrollWidth > maxWidth || span.scrollHeight > maxHeight) && fontSize > 6) {
                fontSize -= 1;
